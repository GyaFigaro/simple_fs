...
int newfs_driver_write(int offset, uint8_t *in_content, int size) {
    int      offset_aligned = NEWFS_ROUND_DOWN(offset, NEWFS_IO_SZ());//aligned对齐的
    int      bias           = offset - offset_aligned;
    int      size_aligned   = NEWFS_ROUND_UP((size + bias), NEWFS_IO_SZ());
    uint8_t* temp_content   = (uint8_t*)malloc(size_aligned);
    uint8_t* cur            = temp_content;
 ...
 * 
 * Layout
 * | Super | Inode Map |Data Map | Inode file | Data file
 * 
 * IO_SZ = BLK_SZ
 * 
 * 每个Inode占用一个Blk
...
int newfs_mount(struct custom_options options){
    ...
    ddriver_ioctl(NEWFS_DRIVER(), IOC_REQ_DEVICE_SIZE,  &newfs_super.sz_disk);//ddriver size
    ddriver_ioctl(NEWFS_DRIVER(), IOC_REQ_DEVICE_IO_SZ, &newfs_super.sz_io);//ddriver io size

    root_dentry = new_dentry("/", NEWFS_DIR);

    if (newfs_driver_read(NEWFS_SUPER_OFS, (uint8_t *)(&newfs_super_d), 
                        sizeof(struct newfs_super_d)) != NEWFS_ERROR_NONE) {		//读newfs_super_d块,辨认文件系统是否已经挂载
        return -NEWFS_ERROR_IO;
    }   
                                                      /* 读取super */
    if (newfs_super_d.magic_num != NEWFS_MAGIC_NUM) {     /* 幻数无 */
        ...
        inode_num  =  NEWFS_DISK_SZ() / ((NEWFS_DATA_PER_FILE + NEWFS_INODE_PER_FILE) * NEWFS_IO_SZ());//NEWFS_IO_SZ可能为数据块大小，也可能为磁盘块大小，或者都不是，这里先猜测为数据块大小

        map_inode_blks = NEWFS_ROUND_UP(NEWFS_ROUND_UP(inode_num, UINT32_BITS), NEWFS_IO_SZ()) 
                         / NEWFS_IO_SZ();

        map_data_blks = NEWFS_DATA_PER_FILE * map_inode_blks;
    ...
    if (newfs_driver_read(newfs_super_d.map_inode_offset, (uint8_t *)(newfs_super.map_inode), 
                        NEWFS_BLKS_SZ(newfs_super_d.map_inode_blks)) != NEWFS_ERROR_NONE) {//读newfs_super_d.map_inode_blks
        return -NEWFS_ERROR_IO;
    }
	//读数据位图
    if (newfs_driver_read(newfs_super_d.map_data_offset, (uint8_t *)(newfs_super.map_data), 
                        NEWFS_BLKS_SZ(newfs_super_d.map_data_blks)) != NEWFS_ERROR_NONE) {//读newfs_super_d.map_inode_blks
        return -NEWFS_ERROR_IO;
    }
