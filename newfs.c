...
void* newfs_init(struct fuse_conn_info * conn_info) {
	...
	/* 下面是一个控制设备的示例 */
	//newfs_super.driver_fd = ddriver_open(newfs_options.device);
...
int newfs_mkdir(const char* path, mode_t mode) {
	...
	char* fname;
	struct newfs_dentry* last_dentry = newfs_lookup(path, &is_find, &is_root);			//解析路径
	struct newfs_dentry* dentry;
	struct newfs_inode*  inode;

	if (is_find) {//如果路径存在，则报错
		return -NEWFS_ERROR_EXISTS;
	}
    
	if (NEWFS_IS_REG(last_dentry->inode)) {//如果是文件
...
int newfs_getattr(const char* path, struct stat * newfs_stat) {
	...
	newfs_stat->st_blksize = NEWFS_IO_SZ();//size

	if (is_root) {//如果是根目录
		newfs_stat->st_size	= newfs_super.sz_usage;/* Size of file, in bytes.  */ 
		newfs_stat->st_blocks = NEWFS_DISK_SZ() / NEWFS_IO_SZ();/* Number 512-byte blocks allocated. */
...
int main(int argc, char **argv)
{
	...
	newfs_options.device = strdup("/home/guests/190111026/ddriver");
